...
if QA_ONLY:
    ...
    for nest_id, py_date, row in to_insert:
        preview_row = {
            UNIQUE_ID_FIELD: nest_id,
            "New_Nest": "No",
            **{f: safe_val(row[f]) for f in SURVEY_FIELDS},
...
#
#   Field order:
#       SHAPE@ | Nest_ID | New_Nest | survey fields | Date_Updated | Year_Updated | Final_Status | static ref fields
# ---------------------------------------------------------------------------
...
with arcpy.da.InsertCursor(sdeOspreyReference, insert_fields) as cursor:
    for nest_id, py_date, row in to_insert:
        ...
        survey_values = [safe_val(row[field]) for field in SURVEY_FIELDS]

        cursor.insertRow(
            [geometry, nest_id, "No"]           # New_Nest always "No" for stacked records
            + survey_values
            + [py_date, year_updated, "NA"]      # Date_Updated, Year_Updated, Final_Status
